...
- `data/` which contains all the clean data used to produce the different plots presented in our paper.  the clean data are obtained by cleaning up the data generated by the simulation. for more details please confer the python notebook `data/clean_data.ipynb`
...
Here we have two alternatives of implementing the model we described in our paper: 
   ...
   To run the basic SEIR simulation, please use the following command: 
      ...
      python `ppseir_basic.py` -C <carrying capacity> -Ir <infection rate> -Rr <recovey rate> 
      -T <Epedemic Duration> -N <Population size> -lf <Log folder> -mu <mutation rate>
