...
def partialExtinctionProb(s_b, s_d, u, k, n) : 
    ...
    q = {
        k: (-1./m_k)*lambertw(-m_k*np.exp(-m_k)).real
    } 
    if k >0 : 
        for i in list(reversed(range(k))) : 
            m_i = m_(s_b, s_d, u, i)
            q[i] = (-1/m_i)*lambertw(-m_i*np.exp(-m_i*(1+sum([((u**(j-i))/factorial((j-i),exact=True))*(1-q[int(j)]) 
            for j in np.arange(i+1,k,step=1)])))).real
    p_ext = 1.
