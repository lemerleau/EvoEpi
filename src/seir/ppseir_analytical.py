...
import json
import pp
import analytical_seir 
import analytic as ac

def run (N,T,params,infection_types,init_infected, log_folder, lf): 

    hosts = analytical_seir.init(N,init_infected,infection_types, params)

    epidemic_data, infections, immune_hosts ,ic_ts= analytical_seir.epidemic(hosts, params,infection_types, T, params["C"])  
...
def main() : 
    ...
    states1 =  ac.stateSpace(5,ac.K(0.5,0.05,args.mu))
    states2 =  ac.stateSpace(5,ac.K(0.5,0.9,args.mu))
    ...
    jobs = [(i , job_server.submit(run, (N,T,parameters[i],infection_types,init_infected, log_folders[i],i), (),( "numpy", "random","pandas","os", "json","analytical_seir"))) for i in range(nb_jobs)]
